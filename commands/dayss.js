  ...
  try {
    ...
    $('#acgs-anime-list .acgs-anime-block').each(function () {
      if (!$(this).find('.anime_streams .stream-sites a').text().includes('巴哈')) {
        ...
        let text = ''
        $(this).find('.anime_streams').each(function () {
          if ($(this).text().includes('台灣')) {
            text = $(this).find('.stream-sites a').map(function () {
              return $(this).text()
            }).get().join(',')
          }
        })
        bubble.body.contents[0].contents[0].contents[1].text = text || '台灣無法觀看'

        // bubble.body.contents[0].contents[0].contents[1].text = $(this).find('.anime_info .anime_specs .anime_tag tags').text() || '無'
